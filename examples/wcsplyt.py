...
<div id="split">
    ...
    """
    def init(self):
        self.root=self.shadowRoot.querySelector("#split")
        self._s=None
    ...
    def connectedCallback(self):  # called when rendered in client side
        ...
        self.root.className=mode
        if self._s: self._s.destroy()
        self._s=Split(self.shadowRoot.querySelectorAll("slot"),{
            "direction":mode,
            "sizes": sizes,
            "minSize": [10, 10],
            "onDragEnd": lambda sizes: self.change(sizes),
        })

    def change(self,sizes):
        self.setAttribute("sizes", JSON.stringify(sizes))
        self.dispatchEvent( window.Event('change') )
